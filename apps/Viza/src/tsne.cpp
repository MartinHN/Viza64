...
void TSNE::run(double* X, int N, int D, double* Y, int no_dims, double perplexity, double theta) {
	...
	for(int iter = 0; iter < max_iter; iter++) {
        ...
        if(iter > 0 && (iter % 10 == 0 || iter == max_iter - 1)) {
            ...
            else      C = evaluateError(row_P, col_P, val_P, Y, N, theta,no_dims);  // doing approximate computation here!
            if(iter == 0)
...
double TSNE::evaluateError(double* P, double* Y, int N,int no_dim) {
    
    // Compute the squared Euclidean distance matrix
    double* DD = (double*) malloc(N * N * sizeof(double));
    double* Q = (double*) malloc(N * N * sizeof(double));
    if(DD == NULL || Q == NULL) { printf("Memory allocation failed!\n"); exit(1); }
    computeSquaredEuclideanDistance(Y, N, no_dim, DD);
...
double TSNE::evaluateError(int* row_P, int* col_P, double* val_P, double* Y, int N, double theta,int no_dim)
{
    
    // Get estimate of normalization term
    const int QT_NO_DIMS = no_dim;
    QuadTree* tree = new QuadTree(Y, N);
    double* buff  = (double*)calloc(no_dim, sizeof(double));
    double sum_Q = .0;
